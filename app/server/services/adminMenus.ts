...
type UpsertPayload = Omit<AdminMenuDetails, 'id'> & { id?: string }

const menuStore = new Map<string, AdminMenuDetails>()
...
function seedMenus() {
  ...
  const seededMenus: AdminMenuDetails[] = [
    {
      id: 'main-menu',
      slug: 'main-menu',
      subdomain: 'main-menu',
  ...
  for (const menu of seededMenus) {
    menuStore.set(menu.id, menu)
    reservedSubdomains.add(normalizeSubdomain(menu.subdomain))
  }
}
...
export function listAdminMenus() {
  return Array.from(menuStore.values())
}

export function getAdminMenu(id: string) {
  return menuStore.get(id) ?? null
}

export function isSubdomainReserved(value: string, options?: { ignore?: string | string[] }) {
  const normalized = normalizeSubdomain(value)

  const ignoreValues = Array.isArray(options?.ignore)
    ? options?.ignore
    : options?.ignore
      ? [options.ignore]
      : []

  const ignoreSet = new Set(ignoreValues.map(normalizeSubdomain))

  if (ignoreSet.has(normalized)) {
    return false
  }

  return reservedSubdomains.has(normalized)
}

export function reserveSubdomain(value: string) {
  reservedSubdomains.add(normalizeSubdomain(value))
}

export function createAdminMenu(payload: UpsertPayload) {
  const normalizedSubdomain = normalizeSubdomain(payload.subdomain)

  if (isSubdomainReserved(normalizedSubdomain)) {
    throw Object.assign(new Error('Subdomain already exists.'), {
      code: 'subdomain_conflict',
      subdomain: normalizedSubdomain
    })
  }

  const id = payload.id ?? (normalizedSubdomain || randomUUID())
  const menu: AdminMenuDetails = {
    ...payload,
    id,
    subdomain: normalizedSubdomain
  }

  menuStore.set(id, menu)
  reserveSubdomain(normalizedSubdomain)

  return menu
}

export function updateAdminMenu(id: string, payload: UpsertPayload) {
  const existing = menuStore.get(id)

  if (!existing) {
    throw Object.assign(new Error('Menu not found.'), { code: 'menu_not_found' })
  }

  const normalizedSubdomain = normalizeSubdomain(payload.subdomain)

  if (normalizedSubdomain !== existing.subdomain && isSubdomainReserved(normalizedSubdomain)) {
    throw Object.assign(new Error('Subdomain already exists.'), {
      code: 'subdomain_conflict',
      subdomain: normalizedSubdomain
    })
  ...
  const updated: AdminMenuDetails = {
    ...existing,
    ...payload,
    id: existing.id,
    subdomain: normalizedSubdomain,
...
export function listReservedSubdomains() {
  return Array.from(reservedSubdomains.values())
}

export function assertSubdomainAvailable(value: string, options?: { ignore?: string | string[] }) {
  if (isSubdomainReserved(value, options)) {
    const error = new Error('Subdomain already exists.')
    ;(error as any).code = 'subdomain_conflict'
    throw error
  }
}
